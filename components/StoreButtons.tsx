  ...
  return (
    ...
    >
      ...
      <span className="text-left">
        <span className="block text-xs font-medium uppercase tracking-[0.18em] text-white/58">{eyebrow}</span>
...
export function StoreButtons({ className = "" }: { className?: string }) {
  return (
    <div className={`flex flex-col gap-3 sm:flex-row ${className}`}>
      <StoreButton eyebrow="Download on the" label="App Store" icon={Smartphone} />
      <StoreButton eyebrow="Get it on" label="Google Play" icon={Play} />
    </div>
  );
}
